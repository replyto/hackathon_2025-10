...
Först behöver vi definiera en class som beskriver datat vi vill omvandla:
...
Kopiera in följande kod i *src/main/java/se/replyto/hackathon/data/__User.java__*:
...
Sedan behöver vi lägga in en Bindy data definition ovanför våran main rutt i *src/main/java/se/replyto/hackathon/routbuilders/**RoutBuilder01.java***:

```java
DataFormat bindy = new BindyCsvDataFormat(User.class);
```

För att kunna börja använda detta, behöver vi dessutom lägga till följande importer:
...
Nu är allt uppsatt för att kunna börja använda våran nya Bindy definition.
...
Innan vi kan påvköra detta behöver vi göra ett sista tillägg: På `from` raden i våran main rutt behöver vi lägga till `*.csv` så att även CSV filer plåckas upp.

```java
from("file:demo/int001-in?antInclude=*.txt,*.csv")
```

Med alla våra ändringar och tilllägg bör *src/main/java/se/replyto/hackathon/routbuilders/**RoutBuilder01.java*** nu se ut ungefär såhär:
...
Nu är vi redå att testa våra ändringar. Stoppa och starta om applikationen.
...
Lägg till följande kod efter våran bindy definition i *RoutBuilder01.java*:
...
Ny behöver vi byta ut `.marhal(bindy)` mot följande kod:
...
Innan vi kan använda våra ändringar behöver vi även importera dem in i klassen:
...
Rensa bort alla filer från *demo/int001-out*.

Kopiera **test.csv** filen från projektets rutt till *demo/int001-in* katalogen.

Kontrollera filen i *demo/int001-out*.
...
Givetvis behöver vi även lägga till `*.json` i `from` för våran main rutt:
...
Kopiera ***test.json*** filen från *demo/int001-out* katalogen in till *demo/int001-in* katalogen.

Kontrollera nya CSV filen i *demo/int001-out*.
...
Skapa en ny fil **RoutBuilder02.java** i *src/main/java/se/replyto/hackathon/routbuilders/* katalogen.
...
Denna fil innehåller del kod vi redan känner igen:
...
Lägg till följande kod för att hämta alla användare:
...
Låt oss nu lägga till rutten för att skapa en nu användare:
...
Vi utgåt från att vi får in User.class data
...
Efter det sparar vi användaren till `userMap`

Setter användaren med det eventuellt nya Id:et till meddelande body:n

till slut sätter vi rätt HTTP status kod som ska retuneras till klienten.

Nästa rutt vi behöver lägga till är den för att hämta en enskild användare:
...
Nästa rutt vi lägger till är den för att ändra en befintlig användare:
...
Till sist lägger vi till rutten för att radera en användare:
...
Den färdiga klassen bör se ut så här:
...
Nu kan vi starta om applicationen om och testa API:et.

Du kan använda curl eller postman för att anropa det på `127.0.0.1:8080/camel/users/`
...
Först behöver vi skapa rest definitionen.
...
Och precis som vi gjorde med API:et, behöver vi skapa den korresponderande rutten:
...
Nu ser *src/main/java/se/replyto/hackathon/routbuilders/__RoutBuilder02.java__* ut så här:
...
Starta om applikationen och öppna [User Management](http://localhost:8080/camel/) i en webläsare.
...
Lägg till följande rutt i *src/main/java/se/replyto/hackathon/routbuilders/__RoutBuilder01.java__*:
 ...
 *src/main/java/se/replyto/hackathon/routbuilders/__RoutBuilder01.java__* ser nu ut såhär:

```java

```

Om du lägger någon av ***test.json*** eller ***test.csv*** i *demo/int002-in*, kan du se hur det hanteras av vårat backend. Om du laddar om [User Management](http://localhost:8080/camel/) kan du sedan se det nya användarna som systemet har lagt till.
